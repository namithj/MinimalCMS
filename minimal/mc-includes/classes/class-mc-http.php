...
class MC_Http
{
	 ...
	 *
	 * @param string $url     Destination URL.
	 * @param string $default Fallback URL when $url is not allowed.
	 * @param int    $status  HTTP status code.
	 * @return never
	 */
	public function safe_redirect(string $url, string $default = '', int $status = 302): never
	{
		$default = '' !== $default ? $default : mc_site_url();
		...
		$host_allow = array_values(array_unique($host_allow));
		$parsed     = parse_url($target);

		if (false === $parsed || str_starts_with($target, '//')) {
			$target = $default;
		} elseif (isset($parsed['host'])) {
			$target_host = strtolower((string) $parsed['host']);

			if (!in_array($target_host, $host_allow, true)) {
				$target = $default;
			}
	...
	public function send_security_headers(): void
	{
		...
		/**
		 * Filter baseline security headers.
		 *
		 * @since {version}
		 *
		 * @param array<string, string> $headers Header map.
		 */
		$headers = $this->hooks->apply_filters('mc_security_headers', $headers);
