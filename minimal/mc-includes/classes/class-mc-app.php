...
class MC_App
{
	...
	public function boot(string $config_path): void
	{
		...
		$core_dir    = defined('MC_CORE_DIR') ? MC_CORE_DIR : $abspath . 'minimal/';
		...
		$data_rel = rtrim($config->get('data_dir', 'content/data'), '/');
		$data_dir = $abspath . $data_rel . '/';
		...
		$content_rel = rtrim($config->get('content_dir', 'content'), '/');
		...
		$content_types  = new MC_Content_Type_Registry($hooks, $content_data_dir);
		$this->set('content_types', $content_types);

		// 10. Content Manager.
		$content_manager = new MC_Content_Manager($content_types, $hooks, $cache, $formatter, $content_data_dir);
		...
		$this->define_constants($config, $abspath, $core_dir, $content_rel, $data_rel, $secret_key, $encryption_key);
		...
		$hooks->add_action(
			...
			static function (string $page_slug, array $values) use ($config): void {
				...
				$config_keys = array(
					...
					'front_page', 'posts_per_page', 'permalink_structure', 'debug', 'active_admin_theme',
				);
	 ...
	 * @param string    $core_dir       Absolute path to core directory.
	 * @param string    $content_rel    Relative content directory name.
	 * @param string    $data_rel       Relative data directory name.
	 * @param string    $secret_key     HMAC secret key from keystore.
	 * @param string    $encryption_key Encryption key from keystore.
	 * @return void
	 */
	private function define_constants(MC_Config $config, string $abspath, string $core_dir, string $content_rel, string $data_rel, string $secret_key = '', string $encryption_key = ''): void
	{

		$content_dir = $abspath . $content_rel . '/';
		$data_dir    = $abspath . $data_rel . '/';
		$admin_route = trim((string) $config->get('admin_route', 'admin'), '/');

		$this->maybe_define('MC_CORE_DIR', $core_dir);
		$this->maybe_define('MC_CORE_REL_PATH', trim(str_replace($abspath, '', $core_dir), '/'));
		$this->maybe_define('MC_INC', $core_dir . 'mc-includes/');
		$this->maybe_define('MC_CONTENT_DIR', $content_dir);
		$this->maybe_define('MC_DATA_DIR', $data_dir);
		$this->maybe_define('MC_CONTENT_DATA_DIR', $data_dir);
		...
		$this->maybe_define('MC_ADMIN_THEMES_DIR', $core_dir . 'admin-themes/');
		$this->maybe_define('MC_ADMIN_ROUTE', $admin_route);
		...
		$this->maybe_define('MC_ACTIVE_ADMIN_THEME', (string) $config->get('active_admin_theme', 'default'));
		...
		if ('' !== $site_url) {
			...
			// Strip the CMS-internal portion (index.php or {admin-route}/*) to
			// isolate the subdirectory prefix from SCRIPT_NAME.
			$base_path = preg_replace('#/(?:index\.php|' . preg_quote($admin_route, '#') . '/.*)$#', '', $script);
			if ('/' === $base_path || '\\' === $base_path || '' === $base_path) {
				$base_path = '';
			}
		}
		$this->maybe_define('MC_BASE_PATH', rtrim($base_path, '/'));
		$this->maybe_define('MC_ADMIN_URL', rtrim((string) $config->get('site_url', ''), '/') . '/' . $admin_route . '/');
	}
