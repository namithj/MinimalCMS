...
class MC_Settings_Registry
{
	...
	public function register_core_pages(): void
	{
		 ...
		 * Storage namespace: core.general → content/data/settings/core.general.php
		 */
		$this->register_page(
			'general',
			array(
	...
	/**
	 * Populate the active_admin_theme select field with discovered themes.
	 *
	 * @since {version}
	 *
	 * @param array<string, array{name:string,path:string}> $themes Installed admin themes keyed by slug.
	 * @return void
	 */
	public function populate_admin_theme_choices(array $themes): void
	{

		$choices = array();
		foreach ($themes as $slug => $theme) {
			$choices[(string) $slug] = (string) ($theme['name'] ?? $slug);
		}
		...
		$this->set_field_choices('general', 'advanced', 'active_admin_theme', $choices);
	}
}
