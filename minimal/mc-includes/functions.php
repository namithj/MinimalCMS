...
{
	$configured = defined('MC_SITE_URL') ? trim((string) MC_SITE_URL) : '';

	if ('' !== $configured) {
		$base = $configured;
	} else {
		$raw_host = (string) ($_SERVER['HTTP_HOST'] ?? '');
		$host     = strtolower(trim(preg_replace('/:\\d+$/', '', $raw_host)));
...
/**
 * Build a URL within the content directory.
 *
 * @since {version}
 *
 * @param string $path Optional relative path inside content.
 * @return string
 */
function mc_content_url(string $path = ''): string
{
	$abspath     = mc_detect_base_path();
	$content_dir = defined('MC_CONTENT_DIR') ? MC_CONTENT_DIR : $abspath . 'content/';
...
{
	$route = defined('MC_ADMIN_ROUTE') ? trim((string) MC_ADMIN_ROUTE, '/') : 'admin';
	$path  = $route;
	...
	return mc_site_url($path);
}

/**
 * Get the active admin theme slug.
 *
 * @since {version}
 *
 * @return string
 */
function mc_get_active_admin_theme(): string
{
	$active = (string) mc_app()->config()->get('active_admin_theme', '');

	if ('' === $active && defined('MC_ACTIVE_ADMIN_THEME')) {
		$active = (string) MC_ACTIVE_ADMIN_THEME;
	}
	...
	$themes = mc_discover_admin_themes();
	if (isset($themes[$active])) {
		return $active;
	}

	return isset($themes['default']) ? 'default' : array_key_first($themes) ?? 'default';
}

/**
 * Discover available admin themes from the core admin-themes directory.
 *
 * @since {version}
 *
 * @return array<string, array{name:string,path:string}>
 */
function mc_discover_admin_themes(): array
{
	$themes_dir = defined('MC_ADMIN_THEMES_DIR')
		? (string) MC_ADMIN_THEMES_DIR
		: (defined('MC_CORE_DIR') ? MC_CORE_DIR . 'admin-themes/' : mc_detect_base_path() . 'minimal/admin-themes/');
	...
	foreach ($entries as $entry) {
		...
		$slug = mc_sanitize_slug((string) $entry);
		if ('' === $slug || '.' === $slug[0]) {
			continue;
		}

		$theme_path = trailingslashit($themes_dir) . $slug . '/';
		if (!is_dir($theme_path)) {
			continue;
		}

		if (!is_file($theme_path . 'templates/admin-header.php') || !is_file($theme_path . 'templates/admin-footer.php')) {
			continue;
		}
...
/**
 * Build a URL within the active admin theme directory.
 *
 * @since {version}
 *
 ...
 * @return string
 */
function mc_admin_theme_url(string $path = ''): string
{
	$slug     = mc_get_active_admin_theme();
	$core_rel = defined('MC_CORE_REL_PATH') ? trim((string) MC_CORE_REL_PATH, '/') : 'minimal';
	$base     = $core_rel . '/admin-themes/' . $slug;

	if ('' === $path) {
		return mc_site_url($base . '/');
	}

	return mc_site_url($base . '/' . ltrim($path, '/'));
}

/**
 * Resolve an admin theme template path.
 *
 * @since {version}
 *
 * @param string $template Relative template path inside admin theme.
 * @return string|false
 */
function mc_get_admin_theme_template(string $template): string|false
{
	$core_dir   = defined('MC_CORE_DIR') ? MC_CORE_DIR : mc_detect_base_path() . 'minimal/';
	$themes_dir = defined('MC_ADMIN_THEMES_DIR') ? MC_ADMIN_THEMES_DIR : $core_dir . 'admin-themes/';
	$active     = mc_get_active_admin_theme();
	$active_tpl = $themes_dir . $active . '/' . ltrim($template, '/');

	if (is_file($active_tpl)) {
		return $active_tpl;
	}

	$default_tpl = $themes_dir . 'default/' . ltrim($template, '/');
	if (is_file($default_tpl)) {
		return $default_tpl;
	}
...
/**
 * Resolve an admin theme page template path.
 *
 * @since {version}
 *
 * @param string $page_file Admin page file name.
 * @return string|false
 */
function mc_get_admin_theme_page_template(string $page_file): string|false
{
	$sanitized = basename(rawurldecode(str_replace('\\', '/', $page_file)));

	if (!preg_match('/^[a-z0-9][a-z0-9\-]*\.php$/i', $sanitized)) {
		return false;
	}

	return mc_get_admin_theme_template('pages/' . $sanitized);
}
...
/**
 * Populate the active_admin_theme select field with installed admin themes.
 *
 * @since {version}
 *
 * @return void
 */
function mc_populate_admin_theme_choices(): void
{
	mc_app()->settings_registry()->populate_admin_theme_choices(mc_discover_admin_themes());
}
